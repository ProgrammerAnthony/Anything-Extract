...
- Scope: loader 扩展，不引入额外依赖服务。
- Backend: `DocumentParser` 支持 `pdf, docx, txt, md, csv, json, xlsx, pptx, eml`。
- Loader：新增 csv、json、markdown 等解析器。
- 上传校验：按扩展名校验（MIME 不匹配时告警）；前端上传与知识库文档页接受相同扩展名。
- 存储架构不变：SQLite + LanceDB。

### Stage 2 Update (2026-02-13)

- Scope: 队列工作器解耦上传与解析/向量化。
- 队列模型：SQLite 表 `document_ingest_jobs`，状态机（queued/processing/completed/failed）、重试与 worker 锁。
- 上传 API：`POST /api/documents/upload` 支持 `processing_mode`（queue/immediate），返回 `ingest_job`。
- Worker：入队 worker 与编排服务；重试 API `POST /api/documents/{document_id}/retry`。
- 前端：知识库文档列表支持处理模式切换、乐观插入、轮询队列状态、失败重试。
- 运行：`run.sh` 默认启动 backend + frontend + ingest worker，可选队列/worker 回退。

### Stage 3 Update (2026-02-14)

- Scope: 可选 OCR/PDF 解析服务层与回退策略。
- 解析策略：`DocumentParser` 支持 `parser_mode=local|server|hybrid`，hybrid 先调服务再回退本地。
- 解析桥接：解析桥接服务与 `/api/system/config` 中解析开关、URL、模式、模型来源。
- 上传范围：支持 `jpg/jpeg/png`，经 OCR 服务或本地回退解析。
- 前端：系统设置页 `/settings` 配置解析开关与策略；文档详情展示解析策略元数据。
- 运行：`run.sh` 支持 `--with-ocr-server`、`--with-pdf-server` 等，依赖服务不可用时回退本地解析。
