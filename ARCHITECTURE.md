...
- **智能检索**：基于向量数据库执行基础检索，可按标签启用 RAG 查询增强
...
当前仅保留一种检索方案：
- **基础向量检索（basic）**：默认且唯一检索方法，基于余弦相似度

在基础检索之上，系统支持一个可插拔的 **RAG 标签增强层**：
- 由 LLM 将每个标签增强为 3 个问题（可刷新）
- 每个标签独立维护 `base_query + enhanced_questions`
- 每个标签的检索仍独立执行，并聚合去重后服务于该标签提取
- 后续可在不改动提取主链路的前提下扩展高级 RAG 策略
