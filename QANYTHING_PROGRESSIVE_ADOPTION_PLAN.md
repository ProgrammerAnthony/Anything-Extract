...
- 参考源：`QAnything`
- 当前日期：2026-02-13
- 当前状态：规划阶段（未执行代码改造）,故不用考虑历史数据兼容
- 适用范围：本地部署优先，CPU 运行优先
- 本项目位于：`D:\AIForMoney\anything-pdf\anything-extract` 
- 本项目对应的架构文档位于：`D:\AIForMoney\anything-pdf\anything-extract\ARCHITECTURE.md` 
- 参考项目位于： `D:\AIForMoney\anything-pdf\QAnything` （anything-extract和QAnything 是两个单独的git项目）
...
4. 第三阶段引入 `ocr_server` + `pdf_parser_server`，并搞清底层模型机制与启动方式。针对需要ocr和pdf识别的内容，会根据类型让用户选择是否选用对应的server进行处理，便于用户进行测试。
...
## 阶段 1：仅引入多类型 loader（不引入 dependent_server）状态：已完成
...
## 阶段 2：引入上传任务服务（复用 insert_files_serve 思路）状态：已完成
...
## 阶段 3：引入 OCR Server + PDF Parser Server 状态：已完成
...
2. `local-model` 模式（最好是整理到 run.sh文件中）：
   - 用户自备模型目录并配置路径。
...
目标：尽量复用 `anything-extract/run.sh` 的“一键启动”。
...
## 阶段 4：引入 rerank（用于知识提取链路） 状态：待完成
